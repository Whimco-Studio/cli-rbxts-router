...
async function formatFile(filePath) {
  // 1) Load any local .prettierrc (async)
  const config = (await prettier.resolveConfig(filePath)) ?? {};
  // 2) Tell Prettier which filepath we’re formatting (for parser inference)
  config.filepath = filePath;
  // 3) Read, format, write
  const raw = fs.readFileSync(filePath, "utf8");
  const formatted = prettier.format(raw, config);
...
export function generateClient() {
  ...
  for (const file of urlFiles) {
    ...
    // ————————————————————————————————————
    const crudAllRegex =
      /generateCrudRoutes\(\s*['"]([^'"]+)['"]\s*,\s*(\{[\s\S]*?\})\s*,/g;
    let crudAllMatch;
    ...
    const singleRouteRegex =
      /registerRoute\(\s*defineRoute\(([^,]+),\s*{[\s\S]*?name\s*:\s*["']([^"']+)["']/g;
    let singleMatch;
